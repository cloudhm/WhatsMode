...
import com.whatsmode.shopify.ui.helper.SwipeLoadMoreTrigger;
import com.whatsmode.shopify.ui.helper.SwipeRefreshTrigger;
import com.whatsmode.shopify.ui.helper.SwipeTrigger;
...
public class SwipeToLoadLayout extends ViewGroup {
                    ...
                    public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                        ...
                        if(recyclerView.getLayoutManager() instanceof LinearLayoutManager){
                            ...
                            if (newState == RecyclerView.SCROLL_STATE_IDLE) {
                                ...
                                if (lastVisibleItem == (totalItemCount - 1)&&mLoadMoreEnabled
                                        &&mAutoLoadMore&&totalItemCount>=10) {
                                    setLoadingMore(true);
                                }
                            }
                        }
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (STATUS.isSwipingToRefresh(mStatus) || STATUS.isSwipingToLoadMore(mStatus) ||
                    ...
                    if (mDebug) {
                        //LogUtil.i(TAG, "Another finger down, abort auto scrolling, let the new finger handle");
                    }
                }
    ...
    public void setRefreshHeaderView(View view) {
        if (view instanceof SwipeRefreshTrigger) {
            ...
            //LogUtil.e(TAG, "Refresh header view must be an implement of SwipeRefreshTrigger");
        }
    }
    ...
    public void setLoadMoreFooterView(View view) {
        if (view instanceof SwipeLoadMoreTrigger) {
           ...
           // LogUtil.e(TAG, "Load more footer view must be an implement of SwipeLoadTrigger");
        }
    }
    ...
    private void updateScroll(final float yScrolled) {
        ...
        if (mDebug) {
            //LogUtil.d(TAG, "mTargetOffset = %d" , mTargetOffset);
        }
        layoutChildren();
        invalidate();
    }
    ...
    private void autoScrollFinished() {
        ...
        if (mDebug) {
            //LogUtil.i(TAG, STATUS.getStatus(mLastStatus) + " -> " + STATUS.getStatus(mStatus));
        }
    }
    ...
    private final static class STATUS {
        ...
        private static void printStatus(int status) {
            //LogUtil.i(TAG, "printStatus:%d" , getStatus(status));
        }
    }
}
